...
## Customising the proxy configuration

The proxy configuration files are generated by Ansible and overwritten on
every playbook run. Put customisations in the files below instead; Ansible
creates them once (or only creates the directory) and never overwrites them.

| Level | nginx | OpenResty | Use for |
|:------|:------|:----------|:--------|
| `server{}` | `/etc/nginx/static/<fqdn>.conf` (`default.conf` without an FQDN) | `/usr/local/openresty/nginx/conf/static/<fqdn>.conf` (`default.conf` without an FQDN) | extra `location` blocks, caching, rewrites, `client_max_body_size`, ... |
| `http{}` | - | `/usr/local/openresty/nginx/conf/http.d/*.conf` | `log_format`, `map`, `lua_shared_dict`, `limit_req_zone`, `proxy_cache_path`, ... |

Files in `http.d/` are included before the `access_log` directive, so a
`log_format` defined there can be used for `perf.log` by setting
`openresty_access_log_format` in the inventory.
